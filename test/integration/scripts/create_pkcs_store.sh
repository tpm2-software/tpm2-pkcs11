...
for t in "label" "wrap-sw"; do
	...
	tpm2_ptool.py addkey --algorithm=aes128 --label="$t" --userpin=myuserpin --path=$TPM2_PKCS11_STORE
	tpm2_ptool.py addkey --algorithm=aes128 --label="$t" --key-label=mykeylabel --userpin=myuserpin --path=$TPM2_PKCS11_STORE
	echo "Added AES Keys"
