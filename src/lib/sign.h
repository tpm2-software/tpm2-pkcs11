...
CK_RV sign_final_ex(CK_SESSION_HANDLE session, unsigned char *signature, unsigned long *signature_len, bool is_oneshot);

static inline CK_RV sign_final(CK_SESSION_HANDLE session, unsigned char *signature, unsigned long *signature_len) {
    return sign_final_ex(session, signature, signature_len, false);
}
